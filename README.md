...
And, of course, you don't want my name at the bottom to show up. You can change it in `_config.yml` file as well as a few other settings like your social links, google analytics, etc.
...
### Resize Images
Use the `resize.sh` script to __automate image resizing and thumbnail generation__:

1. Fork and clone the project to your computer
2. Go inside the project `$ cd photography`
3. Install dependencies: `apt install imagemagick libavif-bin libimage-exiftool-perl`
4. Copy all your pictures (possibly jpg, the largest size available, straight from your camera) and put them inside the `images` directory
5. Run `./resize.sh` and follow the prompts to resize images and generate thumbnails automatically
