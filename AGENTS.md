...
| Repo | `github.com/a-d/pictrace` (origin). PT12 + PT16 are pushed (`e8b2779`). PT11 + the PT12 lightbox follow-up are local commits ahead of `origin/master` — **unpushed** |
...
| Task board | `KANBAN.md` — PT11 done (5 local commits, push pending, rides with the PT12 lightbox fix); PT10 parked; PT13 dormant |
...
- **Image pipeline:** originals → `./resize.sh` (ImageMagick + avifenc + exiftool) → `images/{year}/{NN}_{Location}/fulls` (1024px) + `/thumbs` (512px), JPG + AVIF; EXIF lives in the JPGs — the AVIFs are **metadata-free** (`--ignore-icc --ignore-exif --ignore-xmp`, plain sRGB; the lightbox reads EXIF from the JPG). Since PT11 the script runs `-j` jobs in parallel and has maintenance modes: `--backfill` (missing AVIF from the published JPGs, thumbs first), `--backfill --force` (re-encode in scope) and `--coverage` (report; needs no encoder tools). AVIF coverage: **1907/1907** photos (fulls + thumbs). `rename.sh` stamps EXIF timestamps into filenames (`name~YYYYMMDD_HHMMSS.ext`).
...
- **AVIF metadata is bloat** — avifenc copies ICC/Exif/XMP from the input by default: the pre-PT11 AVIFs carried 12–78 KB of metadata each (worst case 65 KB Exif on a 17.9 KB thumb; 2026 thumbs averaged 2.6× their JPGs). Always pass `--ignore-icc --ignore-exif --ignore-xmp`; the lightbox EXIF panel reads the JPG.
